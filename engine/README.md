...
- `export.reproduce {exportId,outputDir}` 从固定 manifest 和图片/标签/YAML 副本复制，验证每项摘要；项目之后变化不参与重导出。`export.compare {exportId,otherExportId}` 返回 added/removed 的素材 ID 数组、changed 的 `{assetId,fields}` 数组、unchanged 数量、classesChanged 和 formatChanged。新版本保存 manifestHash/labelHash/yamlHash；首轮早期无清单摘要的导出返回 legacy_export_unverified，不能伪称可证明未被外部修改。
- 自定义导出格式：`export.preflight/create` 可带 `formatId`+`formatVersion`（内置预设 `builtin:yolo|coco|voc|csv` 或保存模板）或内联 `format`。规范字段为 `{labelFormat,precision(1~8),naming,layout:{image,classifyImage,label,index},includeDataYaml,cocoFileName,csvBom,csvColumns}`，标签格式决定标签与索引的扩展名，图片路径必须落在 `.png`（导出不做转码）。`{split}` 使索引按划分各生成一份。`export.format.list/get/save/delete` 管理版本化模板（保存为扁平定义，node 级校验在引擎）。任务兼容：yolo/csv 支持五种任务，coco 支持 detect/segment/pose，voc 仅 detect，不兼容组合以 export_format_task_unsupported 拒绝。清单升级为 schemaVersion 2，新增 `format` 与 `auxiliaryFiles:[{path,hash}]`，逐图条目按需记录 label/labelHash；复现按清单路径逐项复制校验，旧清单回退 YOLO 约定，删除模板后历史副本仍可复现。schema 升级到 8，新增 export_formats 表。Agent 只读格式清单并引用既有 formatId，不能自造目录模板。
