...
final class LocalRuntime implements AutoCloseable {
    final LocalModels models;private final Path worker;private final String vocabularyCache,textEncoder;
    ...
    final class Slot {
        ...
        Slot(String device){this.device=device;bridge=new LocalInference(python,worker,()->cancelRequested,vocabularyCache,textEncoder);executor=new ThreadPoolExecutor(1,1,0,TimeUnit.MILLISECONDS,new ArrayBlockingQueue<>(1),Thread.ofPlatform().name("local-"+device+"-",0).factory(),new ThreadPoolExecutor.AbortPolicy());}
    ...
    LocalRuntime(LocalModels models,JsonObject startup)throws Exception{
        ...
        // 词表缓存与文本编码器目录由桌面下发：worker 只读这两个位置，不会自己找地方下载。
        vocabularyCache=directory(startup,"localVocabularyCacheDir");textEncoder=directory(startup,"localTextEncoderDir");
    ...
    /** 启动参数里的可选目录：缺失或不是绝对路径时返回 null，worker 侧按「未配置」处理。 */
    private static String directory(JsonObject startup,String field){if(!startup.has(field)||startup.get(field).isJsonNull())return null;String value=Json.str(startup,field,"");return value!=null&&Path.of(value).isAbsolute()?value:null;}
    ...
    JsonObject load(Slot slot,JsonObject model,long timeout)throws Exception{
        ...
        // 开放词汇必须在这里就告诉 worker：它决定 worker 按 YOLO-World 构造，也决定运行时是否接受 textClasses。
        // 漏掉这一步的话模型会被当普通 YOLO 载入，登记与冻结预检都过了，却在真正推理时才被拒。
        JsonObject loadParameters=Json.obj("modelPath",model.get("modelPath"),"expectedModelHash",model.get("modelHash"),"taskType",model.get("taskType"),"device",slot.device);
        if(Json.bool(model,"openVocabulary",false))loadParameters.addProperty("openVocabulary",true);
        JsonObject loaded=slot.bridge.load(loadParameters,remaining);loaded.add("environment",probed);loaded.addProperty("modelId",Json.required(model,"id"));loaded.addProperty("modelVersion",Json.integer(model,"version",0));loaded.add("model",model.deepCopy());synchronized(this){slot.loaded=loaded.deepCopy();}
    ...
    JsonObject freeze(JsonObject p,JsonObject project)throws Exception{
        validateParameters(p);JsonObject model=models.snapshot(Json.required(p,"modelId"),p.has("modelVersion")?(int)Costs.integer(p,"modelVersion",1,Integer.MAX_VALUE):null);if(!Json.required(model,"taskType").equals(Json.required(project,"taskType")))throw new ApiError(422,"model_task_mismatch","本地模型任务类型与项目不一致。");
        // 类别来源先判定：这是「参数与模型是否匹配」的问题，不该等授权、加载都过了才报。
        boolean openVocabulary=Json.bool(model,"openVocabulary",false);
        if(openVocabulary){if(Json.array(p,"textClasses").isEmpty())throw new ApiError(422,"vocabulary_required","开放词汇模型必须在本次运行中给出要识别的类别名。");}
        else if(p.has("textClasses"))throw new ApiError(422,"vocabulary_unsupported","该模型自带固定类别表，不能临时改类别名。");
        requireAuthorized(model);JsonObject loaded;String device=device(p);
        ...
        Set<String> expected=new HashSet<>(),targets=new HashSet<>();
        // 开放词汇模型的类别由本次的 textClasses 决定，键集合是 0..N-1；固定类别表模型按载入时的类别表核对。
        if(openVocabulary){
            JsonArray text=Json.array(p,"textClasses");
            for(int index=0;index<text.size();index++)expected.add(Integer.toString(index));
        }else{
            for(JsonElement value:Json.array(loaded,"classes"))expected.add(Json.required(value.getAsJsonObject(),"id"));
        }
        for(JsonElement value:Json.array(project,"classes"))targets.add(Json.required(value.getAsJsonObject(),"id"));JsonObject mapping=Json.object(p,"classMap");if(!mapping.keySet().equals(expected))throw new ApiError(422,"class_map_incomplete","模型每个类别都须明确映射，忽略类别请设置 null。");for(JsonElement value:mapping.asMap().values())if(!value.isJsonNull()&&!targets.contains(value.getAsString()))throw new ApiError(422,"class_map_invalid","类别映射不属于冻结项目。");
    ...
    static void validateParameters(JsonObject p){
        FlowPlans.keys(p,"modelId","modelVersion","device","classMap","textClasses","confidence","iou","imageSize","maxDetections","timeoutMs","reuseEnabled","forceRerun","reuseMaxAgeSeconds");Runs.validateReuse(p);
        // 文本类别：1～200 条、去重、单条 ≤100 字符。这里只做结构校验，能不能用取决于模型是否为开放词汇。
        if(p.has("textClasses")){JsonElement value=p.get("textClasses");if(!value.isJsonArray())throw new ApiError(400,"vocabulary_invalid","文本类别必须为数组。");JsonArray names=value.getAsJsonArray();if(names.isEmpty()||names.size()>200)throw new ApiError(400,"vocabulary_invalid","文本类别须为 1 至 200 条。");Set<String> seen=new HashSet<>();for(JsonElement item:names){if(!item.isJsonPrimitive()||!item.getAsJsonPrimitive().isString())throw new ApiError(400,"vocabulary_invalid","文本类别只能包含字符串。");String name=item.getAsString().strip();if(name.isEmpty()||name.length()>100||!seen.add(name))throw new ApiError(400,"vocabulary_invalid","文本类别不能为空、重复或超过 100 字符。");}}if(p.has("modelId"))FlowPlans.string(p,"modelId",128);if(p.has("modelVersion"))Costs.integer(p,"modelVersion",1,Integer.MAX_VALUE);device(p);timeout(p,120000);if(p.has("classMap")){if(!p.get("classMap").isJsonObject())throw new ApiError(400,"class_map_invalid","类别映射必须为对象。");for(var entry:p.getAsJsonObject("classMap").entrySet())if(entry.getKey().isBlank()||(!entry.getValue().isJsonNull()&&(!entry.getValue().isJsonPrimitive()||!entry.getValue().getAsJsonPrimitive().isString()||entry.getValue().getAsString().isBlank())))throw new ApiError(400,"class_map_invalid","类别映射值必须为类别标识或 null。");}for(String field:List.of("confidence","iou"))if(p.has(field)){JsonElement value=p.get(field);if(!value.isJsonPrimitive()||!value.getAsJsonPrimitive().isNumber()||!Double.isFinite(value.getAsDouble())||value.getAsDouble()<0||value.getAsDouble()>1)throw new ApiError(400,"parameter_invalid",field+" 必须在 0 和 1 之间。");}if(p.has("imageSize"))Costs.integer(p,"imageSize",32,4096);if(p.has("maxDetections"))Costs.integer(p,"maxDetections",1,10000);
    }
