...
final class ExportHistory {
    ...
    JsonObject reproduce(JsonObject p)throws Exception{
        ...
        try{
            ...
            for(String split:List.of("train","val"))if(!type.equals("classify")){Files.createDirectories(temporary.resolve("images/"+split));Files.createDirectories(temporary.resolve("labels/"+split));}else for(int index=0;index<Json.array(source.manifest,"classes").size();index++)Files.createDirectories(temporary.resolve(split+"/"+String.format(Locale.ROOT,"%04d",index)));
            for(JsonElement e:Json.array(source.manifest,"assets")){JsonObject a=e.getAsJsonObject();copyVerified(source.directory,temporary,Json.required(a,"image"),Json.required(a,"contentHash"));
                if(!type.equals("classify")){String label="labels/"+Json.required(a,"split")+"/"+Json.required(a,"assetId")+".txt";copyVerified(source.directory,temporary,label,Json.required(a,"labelHash"));}}
            if(!type.equals("classify"))copyVerified(source.directory,temporary,"data.yaml",Json.required(source.record,"yamlHash"));
    ...
    JsonObject compare(JsonObject p)throws Exception{
        ...
        for(String id:b.keySet())if(!a.containsKey(id))added.add(id);return Json.obj("exportId",left.record.get("id"),"otherExportId",right.record.get("id"),"added",added,"removed",removed,"changed",changed,"unchanged",unchanged,"classesChanged",classesChanged);
    }
