...
final class Thumbnails {
    ...
    static Path file(Path dataRoot, Path source, String contentHash) throws IOException {
        ...
        try {
            ...
            Path temporary = directory.resolve(cached.getFileName() + ".tmp");
            try {
                try (OutputStream out = Files.newOutputStream(temporary, StandardOpenOption.CREATE_NEW); MemoryCacheImageOutputStream buffer = new MemoryCacheImageOutputStream(out)) {
                ...
                try { Files.move(temporary, cached, StandardCopyOption.ATOMIC_MOVE); } catch (FileAlreadyExistsException concurrent) { /* 并发生成时保留先到的那份 */ }
