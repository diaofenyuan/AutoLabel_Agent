...
final class LocalModels {
    ...
    JsonObject register(JsonObject p)throws Exception{
        ...
        // 开放词汇标记决定 load 时是否按 YOLO-World 构造、predict 时是否接受 textClasses；缺省为固定类别表模型。
        if(p.has("openVocabulary")){JsonElement value=p.get("openVocabulary");if(!value.isJsonPrimitive()||!value.getAsJsonPrimitive().isBoolean())throw new ApiError(400,"local_model_open_vocabulary_invalid","开放词汇标记必须为布尔值。");content.addProperty("openVocabulary",value.getAsBoolean());}
    ...
    static JsonObject publicModel(JsonObject resource){JsonObject result=new JsonObject();for(String field:List.of("id","version","kind","name","createdAt","updatedAt"))result.add(field,resource.get(field));JsonObject content=Json.object(resource,"content");for(String field:List.of("taskType","format","fileName","modelHash","sizeBytes"))result.add(field,content.get(field));if(content.has("classNames"))result.add("classNames",Json.array(content,"classNames").deepCopy());if(content.has("origin"))result.add("origin",content.get("origin"));if(content.has("catalogId"))result.add("catalogId",content.get("catalogId"));if(content.has("openVocabulary"))result.add("openVocabulary",content.get("openVocabulary"));return result;}
}
