...
final class LocalModels {
    ...
    static JsonObject publicModel(JsonObject resource){JsonObject result=new JsonObject();for(String field:List.of("id","version","kind","name","createdAt","updatedAt"))result.add(field,resource.get(field));JsonObject content=Json.object(resource,"content");for(String field:List.of("taskType","format","fileName","modelHash","sizeBytes"))result.add(field,content.get(field));if(content.has("classNames"))result.add("classNames",Json.array(content,"classNames").deepCopy());return result;}
}
