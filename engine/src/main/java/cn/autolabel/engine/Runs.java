...
final class Runs implements AutoCloseable {
    ...
    JsonObject get(String id){return store.read(c->view(c,id,true));}
    JsonArray list(String project){return store.read(c->{JsonArray a=new JsonArray();for(JsonObject r:project==null?Store.rows(c,"SELECT id FROM runs ORDER BY rowid DESC LIMIT 100"):Store.rows(c,"SELECT id FROM runs WHERE project_id=? ORDER BY rowid DESC LIMIT 100",project))a.add(view(c,Json.required(r,"id"),false));return a;});}
    JsonObject view(Connection c,String id,boolean samples)throws Exception{
        ...
        Json.object(Json.object(Json.object(run,"snapshot"),"local"),"model").remove("modelPath");if(samples){JsonArray items=new JsonArray();for(JsonObject r:Store.rows(c,"SELECT * FROM samples WHERE run_id=? ORDER BY rowid LIMIT 5000",id)){
