...
final class VideoFrames implements AutoCloseable {
        ...
        try{
            ...
            try(InputStream images=new BufferedInputStream(process.getInputStream(),65536);BufferedWriter manifest=Files.newBufferedWriter(partial,StandardCharsets.UTF_8,StandardOpenOption.CREATE_NEW)){
                while(true){
                    ...
                    if(!readPng(images,png,format.equals("png")?remaining:Media.MAX_FILE,count>=integer(fixed,"maxFrames",1,100000)&&!scene,fixed,job))break;
                    ...
                    if(pending.output!=emitted)throw error(502,"video_frame_mismatch","图片和元数据序号不一致。");emitted++;
                    // 场景变化抽帧的去重：与上一张保留帧的灰度差异低于阈值 → 视为近重复，连同临时文件丢弃，不进清单；
                    // 间隔下限再兜一道（候选按桶取样本就隔开，边界帧可能贴桶线）。超过上限仍是整单失败。
                    if(scene){double seconds=(double)pending.relative*parser.numerator/parser.denominator;
                        if(count>0&&seconds-lastKept<minInterval-1e-9){Files.delete(png);dropped++;continue;}
                        BufferedImage candidate=ImageIO.read(png.toFile());if(candidate==null)throw error(422,"video_image_invalid","抽帧图片不能完整解码。");
                        double[] signature;try{if(candidate.getWidth()!=integer(fixed,"outputWidth",1,20000)||candidate.getHeight()!=integer(fixed,"outputHeight",1,20000))throw error(422,"video_image_invalid","抽帧图片实际尺寸与元数据不一致。");signature=signature(candidate);}finally{candidate.flush();}
            ...
            if(parser.sourceCount==0)throw error(422,"video_empty","视频没有可解码帧。");if(count+dropped!=parser.outputCount||parser.expectedPending()!=0)throw error(502,"video_frame_mismatch","实际帧与时间清单不一致。");
    ...
    private JsonObject prepare(JsonObject request,Job job){
        try{
            ...
            String mode=text(request,"mode",20),format=Json.str(request,"format","png");if(!Set.of("interval","every_n","fps","scene").contains(mode)||!Set.of("png","jpg").contains(format))throw error(400,"video_parameters_invalid","抽帧方式或图片格式不支持。");
            ...
            if(mode.equals("every_n"))fixed.addProperty("everyNFrames",integer(request,"everyNFrames",1,1000000));else if(mode.equals("fps"))fixed.addProperty("targetFps",number(request,"targetFps",0.001,240));else if(mode.equals("scene")){
                // 场景变化抽帧（实测口径）：按 minIntervalSeconds 取候选帧，与上一张保留帧的差异 ≥ sceneThreshold 才留下。
                // 不用 ffmpeg 的 gt(scene,…)：那是「与上一帧的瞬时差」，手持慢移永远过不了阈值（4K 手持实测 0.35～0.05 全段只留首帧）。
                double interval=request.has("minIntervalSeconds")?number(request,"minIntervalSeconds",0.001,600):1d;
                fixed.addProperty("sceneThreshold",request.has("sceneThreshold")?number(request,"sceneThreshold",0.05,1):0.15d);fixed.addProperty("minIntervalSeconds",interval);fixed.addProperty("intervalSeconds",interval);
            }else fixed.addProperty("intervalSeconds",number(request,"intervalSeconds",0.001,604800));
    ...
    private static String filter(JsonObject fixed){
        List<String> ranges=new ArrayList<>();String mode=text(fixed,"mode",20);double rate=mode.equals("fps")?fixed.get("targetFps").getAsDouble():mode.equals("every_n")?0:1/fixed.get("intervalSeconds").getAsDouble();
    ...
    private static final class Parser {
        ...
        void accept(String line){
            ...
            if(source){
                ...
                Frame item=expected.poll();
                if(item==null||n!=outputCount||pts!=item.relative||Integer.parseInt(frame.group(4))!=integer(fixed,"outputWidth",1,20000)||Integer.parseInt(frame.group(5))!=integer(fixed,"outputHeight",1,20000))throw error(502,"video_frame_mismatch","输出帧与实际来源时间或尺寸不能一一对应。");
    ...
    private static Rotation rotation(JsonObject stream){
        JsonArray raw=null;Integer rotation=null;for(JsonElement element:Json.array(stream,"side_data_list")){JsonObject side=element.getAsJsonObject();if(!Json.str(side,"side_data_type","").equals("Display Matrix"))continue;if(raw!=null)throw error(422,"video_rotation_unsupported","视频包含冲突的显示矩阵。");String matrix=text(side,"displaymatrix",2000);List<Long> numbers=new ArrayList<>();for(String line:matrix.strip().split("\\R")){int colon=line.indexOf(':');if(colon<0)throw error(422,"video_rotation_unsupported","无法解释视频显示矩阵。");for(String value:line.substring(colon+1).strip().split("\\s+"))numbers.add(Long.parseLong(value));}if(numbers.size()!=9)throw error(422,"video_rotation_unsupported","视频显示矩阵尺寸无效。");raw=Json.element(numbers).getAsJsonArray();
                // 显示矩阵是 3×3 齐次矩阵，第 2 列（索引 2、5）是平移分量：它随旋转角度按画面尺寸缩放，
                // 手机竖屏等真实视频必然非零（如 2160×65536），且与画面方向无关。只比对旋转/镜像子矩阵
                // （索引 0、1、3、4），并把恒为 Q32 定点的 a33（索引 8）一并校验，跳过整个平移列与
                // 两个恒为 0 的索引 6、7。这样带平移的合法矩阵能通过，镜像与非标准角度仍被拒绝。
                long[][] supported={{65536,0,0,0,65536,0,0,0,1073741824},{0,-65536,0,65536,0,0,0,0,1073741824},{-65536,0,0,0,-65536,0,0,0,1073741824},{0,65536,0,-65536,0,0,0,0,1073741824}};int[] direction={0,1,3,4,8};for(int i=0;i<supported.length;i++){boolean same=true;for(int j:direction)same&=numbers.get(j)==supported[i][j];if(same)rotation=i*90;}if(rotation==null)throw error(422,"video_rotation_unsupported","仅支持无镜像的 0、90、180、270 度视频显示矩阵。");}
    ...
    /** 32×24 灰度签名：去重只看整体画面变化，抗手持抖动的高频噪声。 */
    private static double[] signature(BufferedImage image){
        BufferedImage tiny=new BufferedImage(32,24,BufferedImage.TYPE_INT_RGB);java.awt.Graphics2D graphics=tiny.createGraphics();
        try{graphics.setRenderingHint(java.awt.RenderingHints.KEY_INTERPOLATION,java.awt.RenderingHints.VALUE_INTERPOLATION_BILINEAR);graphics.drawImage(image,0,0,32,24,null);}finally{graphics.dispose();}
