...
3. 词表里没有的类别名 + 本机没有编码器 → 必须报 `vocabulary_encoder_missing`，且不新建任何文件（不联网）；
4. 编码器就位时，未命中的类别名可以现场编码（`encoded`），并写回缓存。
...
def main() -> int:
    ...
    try:
        ...
        if not args.encoder:
            ...
            expect_error(blocked, "vocabulary_encoder_missing", "本机没有编码器时必须明确报错而不是联网")
