...
export default function UpdateSettings({ onDirtyChange }: UpdateSettingsProps){
  ...
  return <section className="settings-section update-settings"><h2>应用更新</h2>{isDemo?<Notice>浏览器演示不提供安装包更新，也不模拟下载。请在桌面应用中配置更新源。</Notice>:<>
    <Field label="更新清单地址" hint="留空可关闭更新。需要由应用发布方提供有效的 HTTPS 更新清单。"><input type="url" value={url} disabled={active} onChange={e=>setUrl(e.target.value)} placeholder="https://…/manifest.json"/></Field><Button disabled={active||!changed} busy={pending==='save'} onClick={()=>void saveUrl()}><Save size={14}/>保存更新地址</Button>
