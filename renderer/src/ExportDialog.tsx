...
export default function ExportDialog({ onClose, initialScope = 'project', onOpenTemplate }: { onClose: () => void; initialScope?: AssetScope; onOpenTemplate?: () => void }) {
  ...
  useEffect(() => {
    ...
    request<Preflight>('export.preflight', { projectId: project!.id, taskType, ...formatPayload(format), ...(range.assetIds ? { assetIds: range.assetIds } : {}) }).then(data => { if (active) setPreflight(data); }).catch(e => { if (active) setError(readableError(e)); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
