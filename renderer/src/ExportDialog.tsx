...
interface Preflight { issues: Array<{ message?: string; severity?: string; code?: string } | string>; summary: unknown }
...
interface ExportDifference {added:string[];removed:string[];changed:Array<{assetId:string;fields:string[]}>;unchanged:number;classesChanged:boolean}
...
export default function ExportDialog({ onClose, initialScope = 'project' }: { onClose: () => void; initialScope?: AssetScope }) {
  ...
  useEffect(() => {
    ...
    request<Preflight>('export.preflight', { projectId: project!.id, taskType, ...(range.assetIds ? { assetIds: range.assetIds } : {}) }).then(data => { if (active) setPreflight(data); }).catch(e => { if (active) setError(errorMessage(e)); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [project?.id, taskType, rangeKey, range.count]);
  async function exportData() {
    setBusy(true); setError('');
    try {
      if (!isDemo && !outputDir) throw new Error('请先选择输出目录。');
      if (!range.count) throw new Error('当前范围为空，无法导出。');
      const data = await request<{ path: string; status: string }>('export.create', { projectId: project!.id, outputDir, taskType, onlyConfirmed, trainRatio, ...(range.assetIds ? { assetIds: range.assetIds } : {}) });
