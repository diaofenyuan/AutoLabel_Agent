...
import { Plus, Server, RefreshCw, Save, KeyRound, CheckCircle2, Circle, ExternalLink, ChevronRight, Eye, EyeOff } from 'lucide-react';
...
export default function AiSettings({initialKind='api'}:{initialKind?:'api'|'local'|'library'}){
  ...
  // 简版表单：默认只留「选服务商 → 填 Key → 读取模型」，其余（协议、高级请求、能力逐项、模型职责）收进手动配置。
  const [quick,setQuick]=useState(true);const [presetId,setPresetId]=useState('');
  ...
  const [roleTests,setRoleTests]=useState<Record<string,Record<string,Test>>>({});
  const [credentialSaved,setCredentialSaved]=useState(false);const [showKey,setShowKey]=useState(false);
  ...
  // 已保存密钥回读：打开接口即回显（默认掩码，眼睛切换明文）；重新选择接口与保存后都会重取。
  useEffect(()=>{let active=true;if(!form.id||isDemo)return;request<{hasCredential:boolean;key:string}>('credential.get',{providerId:form.id}).then(data=>{if(!active)return;setKey(data.hasCredential?data.key:'');setCredentialSaved(data.hasCredential);}).catch(()=>{});return()=>{active=false;};},[form.id,credentialSaved]);
  // 模型职责那一栏要显示「这个模型到底行不行」：直接读接口登记的验证记录，不在界面里另存一份状态。
  useEffect(()=>{let active=true;const wanted=([['chat',roles.chatProviderId,roles.chatModel],['annotation',roles.annotationProviderId,roles.annotationModel]] as const)
    .map(([role,providerId,model])=>[role,providerId,model.trim()] as const).filter(([,providerId,model])=>Boolean(providerId&&model));
    if(!wanted.length){setRoleTests({});return;}
    void Promise.all(wanted.map(([role,providerId,model])=>request<{tests?:Record<string,Test>}>('provider.capabilities',{providerId,model})
      .then(data=>[role,data.tests??{}] as const).catch(()=>[role,{}] as const)))
      .then(entries=>{if(active)setRoleTests(Object.fromEntries(entries));});
    return()=>{active=false;};},[roles.chatProviderId,roles.chatModel,roles.annotationProviderId,roles.annotationModel]);
  ...
  async function save(event:React.FormEvent){event.preventDefault();setBusy('save');setError('');let saved='';try{
  ...
  // 简版表单把「保存」与「读取模型」并成一步：填完 Key 就只剩选模型和验证，不用自己想到再点一次。
  if(saved&&quick&&!isDemo)await listModels(saved);
  }
  // 保存后按 id 直接读取模型列表：刚保存时 form.id 还没回到这次的闭包里，不能读 form.id。
  async function listModels(providerId=form.id){if(!providerId){setError('请先保存接口配置。');return;}setBusy('models');setError('');try{const data=await request<{models:string[]}>('provider.models',{providerId});
    // 引擎把清单登记在接口配置里；刷新后上面的派生值与本组件、对话页模型选择器都会跟着更新。
    await refreshProviders();
    notify(data.models.length?`接口返回 ${data.models.length} 个候选模型，请选一个用于测试。`:'接口没有返回任何模型，请核对地址与权限。',!data.models.length);}catch(e){setError(errorMessage(e));}finally{setBusy('');}}
  /**
   * 一键验证并设为默认模型。
   *
  ...
  /** 选预设只填「接口名称 + 基础地址 + 协议」：模型名不预置，交给接口自己的模型列表，猜一个反而会让人以为已经选好了。 */
  function applyPreset(preset:ProviderPreset){
    setPresetId(preset.id);setError('');
    setForm(f=>({...f,name:preset.name&&(!f.name||providerPresets.some(item=>item.label===f.name))?preset.name:f.name,baseUrl:preset.baseUrl||f.baseUrl,protocol:preset.protocol}));
  }
  const presetRow=<div className="ai-presets" role="group" aria-label="常用服务商">{providerPresets.map(preset=><button key={preset.id} type="button" className={presetId===preset.id?'selected':''} disabled={Boolean(busy)} onClick={()=>applyPreset(preset)}><strong>{preset.label}</strong><small>{preset.note}</small></button>)}</div>;
  const keyField=<Field label="API Key" hint={isDemo?'浏览器演示不保存或发送凭据，请在桌面版本配置。':'凭据通过桌面安全通道保存，不写入普通配置。已保存的密钥会回显，留空保存则保留原值。'}><div className="input-icon"><KeyRound size={15}/><input type={showKey?'text':'password'} disabled={isDemo} autoComplete="new-password" value={key} onChange={e=>setKey(e.target.value)} placeholder={isDemo?'演示模式不可用':key?'已保存，点击眼睛查看明文':credentialSaved?'留空保留原凭据':'输入 API Key'}/>{key&&!isDemo&&<button type="button" className="key-reveal" aria-label={showKey?'隐藏 API Key':'查看 API Key'} aria-pressed={showKey} onClick={()=>setShowKey(v=>!v)}>{showKey?<EyeOff size={15}/>:<Eye size={15}/>}</button>}</div></Field>;
  /**
   * 模型职责的即时提示：标注模型必须能读图并按约定返回结构化结果，对话模型要能调工具才跑得动任务。
   * 只提示与就地验证，不替用户改默认值——选错模型的代价要在这里就说清，而不是等跑标注时才以请求失败收场。
   */
  const roleRequirements={annotation:['image','structured'],chat:['tools']} as const;
  function missingRoleCapabilities(role:'chat'|'annotation'){
    const providerId=roles[`${role}ProviderId`],model=roles[`${role}Model`].trim();
    if(!providerId||!model)return [];
    const tests=roleTests[role]??{};
    return roleRequirements[role].filter(cap=>tests[cap]?.status!=='verified');
  }
  async function verifyRole(role:'chat'|'annotation',capability:string){
    const providerId=roles[`${role}ProviderId`],model=roles[`${role}Model`].trim();
    if(!providerId||!model)return;
    setBusy(`role-${role}-${capability}`);setError('');
    try{const result=await request<Test>('provider.test',{providerId,model,capability});
      setRoleTests(data=>({...data,[role]:{...(data[role]??{}),[capability]:result}}));
      notify(`${capabilityNames[capability]}：${result.status==='verified'?'验证通过':result.message||'尚未验证通过'}`,result.status!=='verified');}
    catch(e){setError(errorMessage(e));}finally{setBusy('');}
  }
  function roleWarning(role:'chat'|'annotation'){
    const missing=missingRoleCapabilities(role);if(!missing.length)return null;
    const text=role==='annotation'
      ?`这个模型还不能标注：${missing.map(cap=>capabilityNames[cap]).join('、')}尚未验证。标注模型要能读图，并按约定返回可以解析的标注结果。`
      :`这个模型还不能执行标注任务：${missing.map(cap=>capabilityNames[cap]).join('、')}尚未验证。未通过时只能普通对话，无法创建标注任务。`;
    return <div className="role-warning"><Notice>{text}</Notice><div className="actions">{missing.map(cap=><Button key={cap} busy={busy===`role-${role}-${cap}`} disabled={Boolean(busy)} onClick={()=>void verifyRole(role,cap)}>验证{capabilityNames[cap]}</Button>)}</div></div>;
  }
  const setupSteps=[
    {label:'保存接口',target:'ai-provider-form',done:Boolean(form.id)},
    {label:'选择测试模型',target:'ai-capabilities',done:Boolean(form.id&&form.model.trim())},
    {label:'通过必需能力',target:'ai-capabilities',done:requiredProviderCapabilities.every(cap=>tests[cap]?.status==='verified')},
    {label:'配置模型职责',target:'ai-roles',done:Boolean(roles.chatProviderId&&roles.chatModel.trim()&&roles.annotationProviderId&&roles.annotationModel.trim())}
  ];
  const completedSteps=setupSteps.filter(step=>step.done).length;
  const currentStep=setupSteps.find(step=>!step.done);
  return <section className="settings-section ai-settings"><div className="section-toolbar"><h2>软件 AI 配置</h2>{kind==='api'&&<Button className="primary" onClick={()=>select()}><Plus size={14}/>添加接口</Button>}</div><p className="muted">接口与凭据、本地模型，以及对话与标注各自使用的默认模型。这里的默认值是新建会话与任务的初值；会话内切换只影响该会话。</p><div className="tabs model-kind-tabs"><button disabled={Boolean(busy)} className={kind==='api'?'selected':''} onClick={()=>setKind('api')}>在线接口</button><button disabled={Boolean(busy)} className={kind==='local'?'selected':''} onClick={()=>setKind('local')}>本地模型</button><button disabled={Boolean(busy)} className={kind==='library'?'selected':''} onClick={()=>setKind('library')}>模型库</button></div>
    {kind==='local'?<LocalModels/>:kind==='library'?<ModelLibrary/>:<><section className="ai-setup-progress" aria-labelledby="ai-setup-progress-title"><div className="ai-setup-progress-head"><div><h3 id="ai-setup-progress-title">自动标注配置进度</h3><p>{currentStep?`还差 ${setupSteps.length-completedSteps} 步：${currentStep.label}`:'已完成，接下来可直接新建对话或标注任务。'}</p></div><strong>{completedSteps}/{setupSteps.length}</strong></div><ol>{setupSteps.map((step,index)=><li key={step.label} className={`${step.done?'done':''} ${currentStep===step?'current':''}`} aria-current={currentStep===step?'step':undefined}><a href={`#${step.target}`} onClick={()=>{if(step.target!=='ai-provider-form')setQuick(false);}}><span className="ai-setup-step-icon">{step.done?<CheckCircle2 size={14}/>:<Circle size={14}/>}</span><span>{index+1}. {step.label}</span></a></li>)}</ol></section><div className="models-layout" id="ai-provider-form"><aside className="provider-list"><h2>接口配置</h2>{providers.length?providers.map(provider=><button key={provider.id} className={form.id===provider.id?'selected':''} onClick={()=>select(provider)}><Server size={17}/><div><strong>{provider.name}</strong><small>{provider.protocol==='responses'?'Responses':'Chat Completions'}</small></div><ChevronRight size={13}/></button>):<div className="provider-empty"><div className="provider-empty-icon"><Server size={20}/><span/></div><strong>还没有接口</strong><p>添加一个 OpenAI 兼容接口后，可读取模型并验证能力。</p></div>}<button className="provider-add" onClick={()=>select()}><Plus size={15}/>添加接口</button></aside>
    <div className="model-content"><form onSubmit={save} className="provider-form"><h2>{form.id?'接口设置':'添加接口'}</h2>{quick?<><p className="muted">先选服务商，填上 API Key 就能用；地址与协议会自动填好。想自己调可以切到手动配置。</p>{presetRow}<div className="field-grid"><Field label="接口名称"><input required value={form.name} onChange={e=>set('name',e.target.value)} placeholder="例如：我的 OpenAI 兼容接口"/></Field><Field label="API 基础地址"><input type="url" required value={form.baseUrl} onChange={e=>set('baseUrl',e.target.value)} placeholder="https://api.example.com/v1" autoComplete="url"/></Field></div>{keyField}{form.id&&modelNames.length>0&&<div className="ai-quick-model"><Field label="用于测试的模型" hint="候选来自该接口登记的模型清单，不代表账号权限或模型能力；也可以手动填写。"><ModelCombobox value={form.model} onChange={v=>set('model',v)} options={modelNames} placeholder="输入或选择模型名称"/></Field><div className="actions"><Button className="primary" type="button" busy={busy==='all'} disabled={!form.model||Boolean(busy)} onClick={()=>void verifyAll()}><CheckCircle2 size={14}/>验证并设为默认模型</Button></div></div>}<button className="text-button advanced-toggle" type="button" onClick={()=>setQuick(false)}>手动配置（协议 / 高级请求 / 能力逐项测试 / 模型职责）<ChevronRight size={13}/></button></>:<><div className="field-grid"><Field label="接口名称"><input required value={form.name} onChange={e=>set('name',e.target.value)} placeholder="例如：我的 OpenAI 兼容接口"/></Field><Field label="接口协议"><select value={form.protocol} onChange={e=>set('protocol',e.target.value)}><option value="chat-completions">Chat Completions</option><option value="responses">Responses</option></select></Field></div><Field label="API 基础地址"><input type="url" required value={form.baseUrl} onChange={e=>set('baseUrl',e.target.value)} placeholder="https://api.example.com/v1" autoComplete="url"/></Field>{keyField}<button className="text-button advanced-toggle" type="button" onClick={()=>{setAdvanced(v=>!v);}}>高级请求配置（并发 / 频率 / 超时 / 重试 / 图片上限）<ChevronRight className={advanced?'rotate-90':''} size={13}/></button>{advanced&&<div className="advanced-fields"><div className="field-grid">{([['concurrency','并发上限',1,32],['requestsPerMinute','每分钟请求数',1,60000],['timeoutMs','超时（毫秒）',1000,600000],['maxRetries','最大重试数',0,6],['maxImages','单次图片上限',1,64]] as const).map(([key,label,min,max])=><Field label={label} key={key} hint={key==='maxImages'?'包括参考图片和目标图片，默认 12 张；此配置不保证服务商支持同样数量。':key==='timeoutMs'?'超过这个时间没返回就判为「结果未知」：请求可能已在服务商那边处理或计费，且不会自动重发。大图与高并发更容易触发，可适当调大。':undefined}><input aria-label={label} type="number" min={min} max={max} value={form[key]} onChange={e=>set(key,Number(e.target.value))}/></Field>)}</div><Field label="附加请求头 · JSON"><textarea className="code" rows={3} value={headers} onChange={e=>setHeaders(e.target.value)}/></Field><Field label="附加参数 · JSON"><textarea className="code" rows={3} value={extra} onChange={e=>setExtra(e.target.value)}/></Field></div>}<div className="pricing-form"><label className="checkbox-row"><input type="checkbox" checked={pricing.enabled} onChange={e=>setPricing(p=>({...p,enabled:e.target.checked}))}/>配置手动费用单价</label>{pricing.enabled&&<><p className="muted tiny">每份接口配置保存一个模型的单价，手填逐字段优先于参考价（参考价整理自服务商官方公开价目，仅用于预估，来源见费用明细标注）。未手填时常见模型按参考价估算；查不到参考价的模型才显示未知。填写 0 表示你明确确认该项免费。旧运行使用创建时的价格快照。</p><div className="field-grid"><Field label="单价对应模型"><input aria-label="单价对应模型" value={pricing.model} onChange={e=>setPricing(p=>({...p,model:e.target.value}))}/></Field><Field label="单价币种"><input aria-label="单价币种" maxLength={3} placeholder="例如 CNY、USD" value={pricing.currency} onChange={e=>setPricing(p=>({...p,currency:e.target.value.toUpperCase()}))}/></Field>{([['inputPerMillion','输入 / 百万 token'],['cachedInputPerMillion','缓存输入 / 百万 token'],['outputPerMillion','输出 / 百万 token']] as const).map(([key,label])=><Field key={key} label={label}><input aria-label={label} type="number" min={0} max={1e9} step="any" placeholder="未配置，金额未知" value={pricing[key]} onChange={e=>setPricing(p=>({...p,[key]:e.target.value}))}/></Field>)}</div></>}</div></>}{error&&<p className="inline-error" role="alert">{error}</p>}<div className="actions"><Button className="primary" type="submit" busy={busy==='save'} disabled={Boolean(busy)&&busy!=='save'}><Save size={14}/>{quick?'保存并读取模型列表':'保存接口'}</Button>{form.id&&!isDemo&&<Button type="button" className="danger" disabled={Boolean(busy)} onClick={()=>setDeletePrompt(providers.find(provider=>provider.id===form.id)??null)}>删除接口</Button>}</div></form>
    {!quick&&<><section className="model-section" id="ai-capabilities"><div className="section-toolbar"><h2>模型与能力验证</h2><Button disabled={!form.id||Boolean(busy)} title={!form.id ? '先在左侧添加或选中一个接口' : undefined} onClick={()=>void listModels()}><RefreshCw size={13}/>获取模型列表</Button></div><Field label="用于测试的模型" hint="候选列表来自接口登记的模型清单，不表示账号权限或模型能力。也可以手动填写模型名称。"><ModelCombobox value={form.model} onChange={v=>set('model',v)} options={modelNames} placeholder="输入或选择模型名称"/></Field><div className="capability-table">{capabilityOrder.map(cap=>{const label=capabilityNames[cap],result=tests[cap];return <div className="capability-row" key={cap}><span>{label}</span><span className={result?.status==='verified'?'good':'muted'}>{result?.status==='verified'?<CheckCircle2 size={14}/>:<Circle size={13}/>} {result?.status==='verified'?'已验证':result?.status==='unsupported'?'不支持':'未验证'}</span><small>{result?.testedAt?new Date(result.testedAt).toLocaleString('zh-CN'):'尚无测试记录'}</small><Button busy={busy===cap} disabled={!form.id||!form.model||Boolean(busy)} onClick={()=>void test(cap)}>测试</Button></div>;})}</div><div className="actions"><Button className="primary" busy={busy==='all'} disabled={!form.id||!form.model||Boolean(busy)} onClick={()=>void verifyAll()}>一键验证并设为默认模型</Button></div>
      <p className="muted tiny">一键验证会并发发送 3 次实际请求、覆盖全部六项能力：连接（免费的模型列表）、文本输入、图片输入、多图输入、结构化输出与工具调用。其中前四项是必需能力，全部通过后把该模型写入对话与标注的默认设置；多图输入与工具调用未通过只提示，不阻断默认模型写入。测试会向所选接口发送实际请求，并可能产生调用费用。「图片输入」与「多图输入」用的是引擎内置的 64×64 小图：验证通过只说明接口接受图片格式，<strong>不代表真实尺寸的大图不会超时</strong>。大图是否可用要结合上面「高级请求配置」里的超时设置，以及任务中心「重试未知样本」一起判断。</p></section>
    <section className="model-section" id="ai-roles"><h2>模型职责</h2><div className="role-grid">{(['chat','annotation'] as const).map(role=><div key={role}><h3>{role==='chat'?'对话模型':'标注模型'}</h3><p className="muted">{role==='chat'?'理解要求、解释规则并组织工具':'接收图片，生成可编辑的候选标注'}</p><Field label="接口"><select value={roles[`${role}ProviderId`]} onChange={e=>setRoles(r=>({...r,[`${role}ProviderId`]:e.target.value}))}><option value="">未配置</option>{providers.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="模型名称"><input value={roles[`${role}Model`]} onChange={e=>setRoles(r=>({...r,[`${role}Model`]:e.target.value}))} placeholder="模型 ID"/></Field>{roleWarning(role)}</div>)}</div><div className="actions"><Button busy={busy==='roles'} onClick={()=>{setBusy('roles');void savePrefs({...prefs,...roles}).then(()=>notify('默认模型职责已保存。')).catch(e=>notify(errorMessage(e),true)).finally(()=>setBusy(''));}}><Save size={14}/>保存模型选择</Button></div></section>
    <Notice><span>未通过<Term name="toolCall" />验证的模型可用于普通对话。标注前，请验证图片与<Term name="structuredOutput" />能力。</span><ExternalLink size={13}/></Notice></>}
    </div></div></>}
