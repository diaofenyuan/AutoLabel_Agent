 ...
 */
export default function ProjectResolveDialog({ title, confirmLabel, projects, suggestName = '', allowExisting = true, onClose, onConfirm }: {
  ...
  const [mode, setMode] = useState<'create' | 'existing'>(allowExisting && matched ? 'existing' : 'create');
  const [name, setName] = useState(suggestName);
  const [selectedId, setSelectedId] = useState(matched?.id ?? projects[0]?.id ?? '');
