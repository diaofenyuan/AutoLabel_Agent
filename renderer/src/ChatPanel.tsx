...
export default function ChatPanel({ compact = false, assetId, sessionId }: { compact?: boolean; assetId?: string; sessionId?: string }) {
  ...
  async function send() {
    ...
    if (!chatConfig.providerId || !chatConfig.model || chatConfig.issues.length) { notify('请先在设置里选择对话接口与对话模型。', true); return; }
