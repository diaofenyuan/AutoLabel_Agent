...
export default function Tasks() {
  ...
  useEffect(()=>{if(!events.length||isDemo||engine.state!=='ready')return;const timer=setTimeout(()=>{void refresh();if(selected)void request<Run>('run.get',{runId:selected.id}).then(setSelected).catch(()=>{});},180);return()=>clearTimeout(timer);},[events.at(-1)?.sequence,selected?.id,refresh,engine.state]);
  /**
   * 运行控制。重试的样本集合按状态显式挑选：
  ...
  async function control(action:string,options:{retryUnknown?:boolean}={}){if(!selected)return;setBusy(true);try{
    ...
    if(action==='retry'){
      ...
      payload.assetIds=(selected.samples??[]).filter(sample=>statuses.includes(sample.status)).map(sample=>sample.assetId);
