...
import { ArrowRight, Plus, FolderOpen, Image, Layers } from 'lucide-react';
...
export function Projects() {
  ...
  // 类别不再预置任何场景：默认留空由用户按自己的数据填写，避免把城市场景当成本软件的默认假设。
  ...
  return <div className="content projects-page">
    ...
    <div className="project-composer"><Composer value={description} onChange={setDescription} onSend={() => { setName(description.split('\n')[0].slice(0,60)); setCreate(true); }} placeholder="描述一个新项目，或者从导入图片开始…"><span><Image size={14} />创建项目时保存为项目描述</span></Composer><div className="suggestions"><Button onClick={() => setCreate(true)}><Plus size={14} />创建项目</Button></div></div>
