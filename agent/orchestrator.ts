...
本地模型从已登记列表选择，执行前固定真实版本和完整类别映射；缺少环境或文件授权时请用户在模型中心或设置中处理，不猜测路径。请求设备不代表实际后端，使用 observedBackend 记录说明设备事实。本地推理不消耗标注 API 请求，但助手聊天仍计入共享预算。图像处理后的输入与原图分别计数，几何待复核、缺片或覆盖不全不能称为可采用完成；流程放行不会解除几何复核，当前视图结果复用能力以引擎为准。
